...
void mp3dec_load_cb(mp3dec_t *dec, mp3dec_io_t *io, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data);
/* iterate through frames */
size_t mp3dec_iterate_buf(const uint8_t *buf, size_t buf_size, MP3D_ITERATE_CB callback, void *user_data);
size_t mp3dec_iterate_cb(mp3dec_io_t *io, MP3D_ITERATE_CB callback, void *user_data);
...
size_t mp3dec_iterate_cb(mp3dec_io_t *io, MP3D_ITERATE_CB callback, void *user_data)
{
    uint8_t buf[MINIMP3_RING_SIZE];
    size_t filled = io->read(buf, sizeof(buf), io->read_data), consumed = 0;
    ...
    do
    {
        ...
        if (callback)
        {
            if (callback(user_data, hdr, frame_size, free_format_bytes, 0, readed, &frame_info))
                break;
        }
        readed += frame_size;
        consumed += i + frame_size;
        if (!eof && filled - consumed < 16*1024)
        {   /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
            memmove(buf, buf + consumed, filled - consumed);
            filled -= consumed;
            consumed = 0;
            size_t readed = io->read(buf, sizeof(buf) - filled, io->read_data);
            if (readed != (sizeof(buf) - filled))
                eof = 1;
            filled += readed;
        }
    } while (1);
...
int mp3dec_ex_open_cb(mp3dec_ex_t *dec, mp3dec_io_t *io, int seek_method)
{
    ...
    if (MP3D_SEEK_TO_SAMPLE == dec->seek_method)
    {
        if (mp3dec_iterate_cb(io, mp3dec_load_index, dec) > 0 && !dec->index.frames && !dec->vbr_tag_found)
            return MP3D_E_MEMORY;
        mp3dec_init(&dec->mp3d);
        dec->buffer_samples = 0;
    }
